...
export class AddcontactPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.activaterouter.params.subscribe(async (params:any) => {
      if(params['key']!="" && params['key']!=null && params['key']!=undefined && params['key']!=0){
        this.select_key = params['key'];
      }
  ...
  async addPhoneNumber(){
    ...
    let tag = "";
    if(this.tagOne!=undefined)
      tag += "'"+this.tagOne+"'";
    if(this.tagTow!=undefined)
      tag += "'"+this.tagOne+"'";
    if(this.tagThree!=undefined)
      tag += "'"+this.tagOne+"'";
    tag+="";
    let num = this.select_key+this.number;
